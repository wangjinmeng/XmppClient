...
//查找聊天记录事件：queryHistory; jid:裸jid
    ...
    function init(){
        ...
        $node.on('click','#js-approve-btn-submit',function () {
            Gab.connection.send($pres({
                to:Gab.pending_subscriber,
                type:'subscribed'
            }));
        ...
        $node.on('click','#js-add-contact',function(){
        ...
        $node.on('click','#js-add-contact-btn-close',function () {
            $('#js-add-contact-name').val('');
            Gab.addContactPopups.hide();
        });
        $node.on('click','#js-add-contact-btn-submit',function () {
        ...
        $node.on('click','.js-contact-items .js-del',function () {
        ...
        $node.bind('disconnected',function(){
            Gab.connection=null
        });
